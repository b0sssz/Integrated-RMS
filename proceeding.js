	...
	// Your web app's Firebase configuration
		...
		var User;
		var Folder;
		storageRef1 = storage.ref("Proceeding/" );
		var storageRef;						
		var uploadtask ;
		var title1 ="";
		var author ="";
		var coAuthor ="";
		var conference ="";
		var ProceedingLocation = "";
		var ProceedingDate = "";
		var ProceedingJournal ="";
		var html;
		var file;
...
const bar = document.querySelector('.progress');
                ...
                uploadfile.addEventListener('click', function(e) {
                 ...
                 if (User){
					...
					conference = document.getElementById('conference').value.toUpperCase();	
				    ProceedingLocation = document.getElementById('Proceeding_location').value.toUpperCase();	
					ProceedingDate = document.getElementById('Proceeding_date').value;	
					ProceedingJournal = document.getElementById('Proceeding_Journal').value.toUpperCase();	
...
function savefile(){
	if (User) {
		...
		var uploadref = storage.ref('Proceeding/'+ User.uid+'/'+ title1+'/'+ file.name+'/');
		...
		uploadref.getDownloadURL().then(function(url){
			...
			//console.log(Fileurl);
			 ...
			 db.collection('Proceeding/'+'UserID/' + User.uid ).doc().set({
			
              Proceeding_Name : title1,
			  Author : author,
			  Co_Author : coAuthor,
			  Conference_Name : conference,
			  File_Link : Fileurl,
			  Proceeding_Location : ProceedingLocation,
			  Proceeding_Date : ProceedingDate,
			  Proceeding_Journal : ProceedingJournal
...
function listfile(){
	if (User) {
		...
		db.collection('Proceeding/'+'UserID/' + User.uid ).orderBy('Proceeding_Date', "desc").get().then(function(doc) {
			...
			doc.forEach(function(fileRef){
				...
				<tr class="filerow" >
				<td>${i+1}</td>
				<td  ><a href = "${fileRef.data().File_Link}" >${fileRef.data().Proceeding_Name}</td>
				...
				<td colspan="2">
					...
					<input type="button"  data-id1 = ${fileRef.id} data-id2 = ${fileRef.data().File_Link} data-id3 = ${fileRef.data().Proceeding_Name} id="delete${+i}"  value = "Delete"/>
																...
																(e => alert(e.message));
																	...
																	if (confirm("Are you sure to delete this file?")) {
																		...
																		filestore.delete().then(function(){
																			db.collection('Proceeding/'+'UserID/' + User.uid ).doc(id).delete().then(()=>{
																		  location.reload();
																		   });
																		}).catch(err => {
																		  db.collection('Proceeding/'+'UserID/' + User.uid ).doc(id).delete().then(()=>{
...
//---------------------------------------edit list file into firestore--------------------------------------------------------------------------//
															...
															editbtn.addEventListener('click', (e) => {
																...
																db.collection('Proceeding/'+'UserID/' + User.uid ).doc(id).get().then(function(doc) {
																	const html = `
	
																	<div class="modal2display">
			
																	<div class="subject_input">
																	 <label for="title"> Proceeding Title</label>
																	 <input type="text" value = "${doc.data().Proceeding_Name}" name="title" id="updatetitle" required>  
																	 ...
																	 <label for="title"> Proceeding Location</label>
																     <input type="text" value="${doc.data().Proceeding_Location}" name="title" id="updateProceeding_location" required>
																	 <label for="title">Journal Name </label> 
																	 <input type="text" value = "${doc.data().Proceeding_Journal}" id="updateProceeding_Journal" name="ProceedingJournal" required>          
																	 <label for="title">Date: </label> 
																	 <input type="date" value = "${doc.data().Proceeding_Date}" id="updateProceeding_date" name="ProceedingDate" required>    
                                                                   ...
                                                                   e.preventDefault;
																	...
																	Uplocation = document.getElementById('updateProceeding_location').value.toUpperCase();
																	UpCdate = document.getElementById('updateProceeding_date').value.toUpperCase();
																	UpCJournal = document.getElementById('updateProceeding_Journal').value.toUpperCase();

																	db.collection('Proceeding/'+'UserID/' + User.uid ).doc(id).update({
																		Proceeding_Name : Uptitle,
																		Author : UpAuthor,
																		Co_Author : UpCoAuthor,
																		Conference_Name : UpConference,
			                                                            Proceeding_Location : Uplocation,
																		Proceeding_Date : UpCdate,
																		Proceeding_Journal : UpCJournal
